...
func TestWindowAggregator(t *testing.T) {
	...
	require.Equal(t, []*core.Point{
		{0, baseTime.UnixNano(), baseAlignedTime, false},
		{1, baseTime.Add(time.Minute * 5).UnixNano(), baseAlignedTime + windowDur, false},
		{2, baseTime.Add(time.Minute * 10).UnixNano(), baseAlignedTime + windowDur*2, false},
	}, points)
}
